...
def check_time(precision = 0.1):
	...
	while duration > precision:
		try:
			print(u"{}, 开始获取网络时间戳".format( time.time() ))
			start = time.time()
			networkTime = get_network_time()
			end = time.time()
			duration = end-start
		except Exception as e:
			print(u"获取网络时间出了点小状况，正重试", duration)
	...
	print(u"difference = {}, (本地时间戳+difference)=网络时间戳".format(difference))
	return difference
...
def read_config( file_path ):
	...
	try:
		...
		print("{}".format(e))
		cfg = dict()
		print( u"未能正确加载{}，请检查路径，json文档格式，或者忽略此警告".format( file_path ) )
	return cfg
