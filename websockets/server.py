...
class WebSocketServerProtocol(WebSocketCommonProtocol):
    """
    Complete WebSocket server implementation as an :class:`trollius.Protocol`.
    ...
    def connection_made(self, transport):
        ...
        self.handler_task = trollius_ensure_future(
    ...
    def handler(self):
        ...
        try:
            ...
            try:
                for path in self.handshake(origins=self.origins, subprotocols=self.subprotocols,extra_headers=self.extra_headers):
                    yield path
            ...
            try:
                for info in self.ws_handler(self, path):
                    yield info
            except Exception as exc:
                if self._is_server_shutting_down(exc):
                    for info in self.fail_connection(1001):
                        yield info
                else:
    ...
    def _is_server_shutting_down(self, exc):
        ...
        """
        return (
            isinstance(exc, trollius.CancelledError) and
    ...
    def handshake(self, origins=None, subprotocols=None, extra_headers=None):
        ...
        try:
            for path, headers in read_request(self.reader):
                yield path, headers
        ...
        if extra_headers is not None:
            ...
            if isinstance(extra_headers, abc.Mapping):
...
class WebSocketServer(trollius.AbstractServer):
    """
    Wrapper for :class:`~trollius.Server` that triggers the closing handshake.
    ...
    def wrap(self, server):
        """
        Attach to a given :class:`~trollius.Server`.

        Since :meth:`~trollius.BaseEventLoop.create_server` doesn't support
        injecting a custom ``Server`` class, a simple solution that doesn't
        rely on private APIs is to:

        - instantiate a :class:`WebSocketServer`
        - give the protocol factory a reference to that instance
        - call :meth:`~trollius.BaseEventLoop.create_server` with the factory
        - attach the resulting :class:`~trollius.Server` with this method
    ...
    def wait_closed(self):
        ...
        """
        # trollius.wait doesn't accept an empty first argument.
        if self.websockets:
            # The handler or the worker task can terminate first, depending
            # on how the client behaves and the server is implemented.
            for info in trollius.wait([websocket.handler_task for websocket in self.websockets] +[websocket.worker_task for websocket in self.websockets],loop=self.loop):
                yield info
        for info in self.server.wait_closed():
            yield info


@trollius.coroutine
def serve(ws_handler, host=None, port=None, tmp=None,
    ...
    It yields a :class:`~trollius.Server` which provides:

    * a :meth:`~trollius.Server.close` method that closes open connections with
      status code 1001 and stops accepting new connections
    * a :meth:`~trollius.Server.wait_closed` coroutine that waits until closing
    ...
    :meth:`~trollius.BaseEventLoop.create_server` method. ``host``, ``port`` as
    well as extra keyword arguments are passed to
    :meth:`~trollius.BaseEventLoop.create_server`.
    ...
    for server in loop.create_server(factory, host, port, **kwds):
        yield server
