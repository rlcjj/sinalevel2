...
class WebSocketClientProtocol(WebSocketCommonProtocol):
    """
    Complete WebSocket client implementation as an :class:`trollius.Protocol`.
    ...
    def handshake(self, wsuri,
        ...
        try:
            for status_code, headers in read_response(self.reader):
...
def connect(uri, _,
    ...
    :meth:`~trollius.BaseEventLoop.create_connection` method. Extra keyword
    arguments are passed to :meth:`~trollius.BaseEventLoop.create_connection`.
    ...
    * ``subprotocols`` is a list of supported subprotocols in order of
    ...
    for transport, protocol in loop.create_connection(factory, wsuri.host, wsuri.port, **kwds):
        yield transport, protocol

    try:
        for info in protocol.handshake(wsuri, origin=origin, subprotocols=subprotocols, extra_headers=extra_headers):
            yield info
    except Exception:
        for info in protocol.close_connection(force=True):
            yield info
        raise
