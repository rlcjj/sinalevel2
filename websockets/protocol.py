...
from .compatibility import trollius_ensure_future
...
class WebSocketCommonProtocol(trollius.StreamReaderProtocol):
    """
    ...
    (at least in ``trollius`` and in the OS) will fill up, then the TCP receive
    ...
    * as an iterable of (name, value) pairs in the :attr:`raw_request_headers`
    ...
    def __init__(self,_,
        ...
        self.opening_handshake = trollius.Future(loop=loop)
        # Set to True when the closing handshake has completed properly and to
        # False when the connection terminates abnormally.
        self.closing_handshake = trollius.Future(loop=loop)
        # Set to None when the connection state becomes CLOSED.
        self.connection_closed = trollius.Future(loop=loop)

        # Queue of received messages.
        self.messages = trollius.queues.Queue(max_queue, loop=loop)
    ...
    def close(self, code=1000, reason=''):
        ...
        It's safe to wrap this coroutine in :func:`~trollius.ensure_future`
    ...
    def recv(self):
        ...
        except trollius.queues.QueueEmpty:
            pass
        ...
        next_message = trollius_ensure_future(
            self.messages.get(), loop=self.loop)
        try:
            for done, pending in trollius.wait([next_message, self.worker_task],loop=self.loop, return_when=trollius.FIRST_COMPLETED):
                yield done, pending
    ...
    def send(self, data):
        ...
        for info in self.write_frame(opcode, data):
            yield info
    ...
    def ping(self, data=None):
        ...
        It returns a :class:`~trollius.Future` which will be completed when the
        ...
        self.pings[data] = trollius.Future(loop=self.loop)
        for info in self.write_frame(OP_PING, data):
            yield info
        yield self.pings[data]
        return
    ...
    def pong(self, data=b''):
        ...
        for info in self.write_frame(OP_PONG, data):
            yield info
    ...
    def ensure_open(self):
        ...
        if self.state == CLOSING:
            for info in trollius.wait_for(self.worker_task, 3 * self.timeout, loop=self.loop):
                yield info
            raise ConnectionClosed(self.close_code, self.close_reason)
    ...
    def run(self):
        ...
        for info in self.opening_handshake:
            yield info
        while not self.closing_handshake.done():
            try:
                for msg in self.read_message():
                    yield msg
                if msg is None:
                    break
                for info in self.messages.put(msg):
                    yield info
            except trollius.CancelledError:
                break
            except WebSocketProtocolError:
                for info in self.fail_connection(1002):
                    yield info
            except trollius.IncompleteReadError:
                for info in self.fail_connection(1006):
                    yield info
            except UnicodeDecodeError:
                for info in self.fail_connection(1007):
                    yield info
            except PayloadTooBig:
                for info in self.fail_connection(1009):
                    yield info
            except Exception:
                for info in self.fail_connection(1011):
                    yield info
                raise
        for info in self.close_connection():
            yield info

    @trollius.coroutine
    def read_message(self):
        # Reassemble fragmented messages.
        for frame in self.read_data_frame(max_size=self.max_size):
            yield frame
        if frame is None:
            return
        ...
        if frame.fin:
            yield frame.data.decode('utf-8') if text else frame.data
            return
        ...
        while not frame.fin:
            for frame in self.read_data_frame(max_size=max_size[0]):
                yield frame
            if frame is None:
        ...
        yield ('' if text else b'').join(chunks)
        return
    ...
    def read_data_frame(self, max_size):
        ...
        # 6.2. Receiving Data
        while True:
            for frame in self.read_frame(max_size):
                yield frame
            ...
            if frame.opcode == OP_CLOSE:
                ...
                if self.state == OPEN:
                    # 7.1.3. The WebSocket Closing Handshake is Started
                    for info in self.write_frame(OP_CLOSE, frame.data):
                        yield info
                ...
                # Answer pings.
                for info in self.pong(frame.data):
                    yield info
    ...
    def read_frame(self, max_size):
        ...
        for frame in read_frame(self.reader.readexactly, is_masked, max_size=max_size):
            yield frame
    ...
    def write_frame(self, opcode, data=b''):
        ...
        # https://github.com/python/trollius/pull/280
        # https://github.com/python/trollius/pull/291
    ...
    def close_connection(self, force=False):
        ...
        if self.is_client and not force:
            try:
                for info in trollius.wait_for(self.connection_closed, self.timeout, loop=self.loop):
                    yield info
            except (trollius.CancelledError, trollius.TimeoutError):
                pass
    ...
    # trollius StreamReaderProtocol methods
