...
# trollius's debug mode has a 10x performance penalty for this test suite.
if os.environ.get('PYTHONtrolliusDEBUG'):                    # pragma: no cover
    MS *= 10
...
class CommonTests:
    ...
    def make_drain_slow(self):
        ...
        @trollius.coroutine
        def delayed_drain():
            for info in trollius.sleep(3 * MS, loop=self.loop):
                yield info
            for info in original_drain():
                yield info
    ...
    @property
    def async(self):
        return functools.partial(trollius_ensure_future, loop=self.loop)
    ...
    def last_sent_frame(self):
        ...
        """
        stream = trollius.StreamReader(loop=self.loop)
    ...
    def test_recv_cancelled(self):
        ...
        with self.assertRaises(trollius.CancelledError):
