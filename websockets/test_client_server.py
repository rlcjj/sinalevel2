...
def handler(ws, path):
    if path == '/attributes':
        for info in ws.send(repr((ws.host, ws.port, ws.secure))):
            yield info
    elif path == '/headers':
        for info in ws.send(str(ws.request_headers)):
            yield info
        for info in ws.send(str(ws.response_headers)):
            yield info
    elif path == '/raw_headers':
        for info in ws.send(repr(ws.raw_request_headers)):
            yield info
        for info in ws.send(repr(ws.raw_response_headers)):
            yield info
    elif path == '/subprotocol':
        for info in ws.send(repr(ws.subprotocol)):
            yield info
    else:
        for info in ws.send((x for x in ws.recv())):
            yield info
...
class ClientServerTests(unittest.TestCase):
    ...
    def stop_client(self):
        try:
            self.loop.run_until_complete(
                trollius.wait_for(self.client.worker_task, timeout=1))
        except trollius.TimeoutError:                # pragma: no cover
    ...
    def stop_server(self):
        ...
        try:
            self.loop.run_until_complete(
                trollius.wait_for(self.server.wait_closed(), timeout=1))
        except trollius.TimeoutError:                # pragma: no cover
    ...
    @mock.patch('websockets.server.read_request')
    ...
    @mock.patch('websockets.client.read_response')
    ...
    @mock.patch('websockets.client.build_request')
    ...
    @mock.patch('websockets.server.build_response')
    ...
    def test_server_does_not_switch_protocols(self, _read_response):
        ...
        def wrong_read_response(stream):
            for code, headers in read_response(stream):
                yield code, headers
            yield 400, headers
            return
    ...
    @mock.patch('websockets.server.WebSocketServerProtocol.send')
    ...
    @mock.patch('websockets.server.WebSocketServerProtocol.close')
    ...
    @mock.patch.object(WebSocketClientProtocol, 'handshake')
    ...
    def test_server_shuts_down_during_opening_handshake(self, _read_request):
        _read_request.side_effect = trollius.CancelledError
